...
namespace roa {
    ...
    public:
        ...
        // Allow moving
        on_leaving_scope_class(on_leaving_scope_class &&other) :
        ...
        on_leaving_scope_class(const Func &f) :
                m_func(f),
                m_active(true) {
        }

        // Accept rvalue function objects
        on_leaving_scope_class(Func &&f) :
                m_func(std::move(f)),
                m_active(true) {
        }
    ...
    template<typename Func>
    on_leaving_scope_class<typename std::decay<Func>::type>
    on_leaving_scope(Func &&f) {
        return on_leaving_scope_class<typename std::decay<Func>::type>(std::forward<Func>(f));
    }
}
