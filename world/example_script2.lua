...
elseif entity.class_type == roa.class_type.Tile then
    ...
    local min_tile_id = map_entity.get_first_tile_id()
    if entity.get_tile_id() <= min_tile_id then
        roa.create_script("tile_id_upwards", entity.get_id(), 200, 200, roa.script_trigger_type.Looped, false)
        self_script.destroy()
    else
