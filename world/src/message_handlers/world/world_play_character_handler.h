...
namespace roa {
    class world_play_character_handler : public imessage_handler<false> {
    public:
        explicit world_play_character_handler(Config& config, moodycamel::ReaderWriterQueue<std::shared_ptr<player_event>>& player_event_queue,
                iplayers_repository& players_repository, std::shared_ptr<ikafka_producer<false>> producer);
        ~world_play_character_handler() override = default;

        void handle_message(std::unique_ptr<binary_message const> const &msg) override;

        static constexpr uint32_t message_id = play_character_message<false>::id;
    private:
        Config& _config;
